...
uint64_t libjake_find_str(jake_img_t syms,char * str) {
	uint64_t found = (uint64_t)memmem(syms->map,syms->mapsize,str,strlen(str));
	if (found == 0) {
...
uint64_t find_load_skip(jake_img_t syms, uint64_t start_addr, uint64_t n_instr, bool backwards, int toskip) {
	// we search for either adr or adrp + (add/ldr)
	// this also verifies that the input register of ldr/add is the right register
	uint64_t adr_value = 0x0;
	char current_reg = -1;
...
uint64_t find_str_xref(jake_img_t syms, char * str) {
...
uint64_t find_realhost(jake_img_t syms) {
	...
	uint64_t host_priv_self = jake_find_symbol(syms,"_host_priv_self");
	if (!host_priv_self) {
